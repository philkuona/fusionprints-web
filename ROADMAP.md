...
2.4.2 🟡 built 2026-06-06 — AVIF image format enabled (verified serving image/avif live) + WebP fallback; HeroCarousel now lazy-mounts slides (homepage loads ~2 hero images, not 5). Confirmed already-good: per-route code splitting, Konva dynamically imported (not in marketing bundle), exactly one priority image per page, `sizes` on every `<Image>`. Source JPEGs left as-is (next/image serves clients optimized AVIF at display size, so source weight doesn't affect client LCP; recompression is optional repo hygiene). Remaining: a real Lighthouse/PageSpeed perf ≥85 mobile run on the live URL to close the gate.
